...
> For model.ipynb: 
  ...
  - After all images are checked for fitting the tile size, the CNN starts taking form and in ‘early’ the early stopping method from keras.callbacks is 
    used to prevent overfitting (as it reaches a defined __ accuracy(?)). Once reached the desired form, the model is applied and the metrics can be 
    calculated.
